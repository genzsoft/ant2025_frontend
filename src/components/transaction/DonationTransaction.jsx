...
export default function DonationTransaction({ token }) {
  ...
  const fetchData = useCallback(async ({ reset = false } = {}) => {
    ...
    try {
      setError('');
      setLoading(true);
      const url = `${Api_Base_Url}/api/donation/?page=${page}`; // If backend supports paging
      const res = await axios.get(url, {
        headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }
      });
      const { results = [], next = null, count = results.length } = res.data || {};
      setCount(count);
      setNext(next);
      setItems(prev => reset ? results : [...prev, ...results]);
    } catch (err) {
      console.error('[DonationTransaction] fetch error', err);
  ...
  return (
    <div className="mb-10">
      <div className="space-y-4">
        ...
        {items.map((tx) => (
          <div key={tx.id || `${tx.created_at}-${tx.amount}-${tx.user_phone}`} className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 flex flex-col gap-2 text-sm">
            <div className="flex items-center justify-between">
              <div className="text-gray-600 text-sm">Donor: {tx.user_phone}</div>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-green-100 text-green-700 uppercase tracking-wide">Donation</span>
            </div>
        ...
        {!loading && items.length === 0 && !error && (
          <div className="text-center py-10 text-sm text-gray-500 bg-white rounded-lg border border-dashed">No donations found.</div>
        )}
